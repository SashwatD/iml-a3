...
class PowerfulCNNLSTMModel(nn.Module):
    def __init__(self, vocab_size, embed_dim=300, hidden_dim=512, embedding_matrix=None, num_emotions=9, dropout_rate=0.5):
        ...
        # A. Shared Encoder (The Backbone)
        self.cnn = CustomCNN(output_dim=512)
        
        # --- THE FIX: SPLIT HEADS ---
        
        # B. Head 1: The "Content" Path (For Captioning)
        ...
        # C. Head 2: The "Emotion" Path (For Classification)
    ...
    def forward(self, images, captions):
        ...
        # 2. Split Paths
        # Content Vector: "Woman, Beach, Sand"
        content_vec = self.content_projector(shared_feats) # (B, embed_dim)
        
        # Emotion Logits: "Happy, Calm"
        emotion_logits = self.emotion_projector(shared_feats) # (B, num_emotions)
        
        # 3. Caption Generation (Using ONLY Content Vector)
        # We inject the content vector as the first token (Context Primer)
        img_token = content_vec.unsqueeze(1) # (B, 1, embed_dim)
