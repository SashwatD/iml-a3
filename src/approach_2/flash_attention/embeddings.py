...
def get_pretrained_embeddings(vocab_list, model_name="glove-wiki-gigaword-100", embedding_dim=100, captions=None):
    ...
    if model_name == "word2vec" and not os.path.exists(local_path):
        # Fallback to scratch training if strictly requested and no local file
        # But user requested removing gensim, so we can't train easily without it or another lib.
        # Assuming we only use pretrained for now as per instructions.
        print("Word2Vec scratch training requested but gensim is removed. Please use pretrained.")
